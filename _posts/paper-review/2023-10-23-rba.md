...
tags:
  ...
  - ICLR
  - '2023'
excerpt: "paper review about RBA"
use_math: true
classes: wide
---
...
* 위 두 방식에서 수행된 behavior에 대한 표현력은 제한적이었으며, 이러한 한계점을 해결하기 위해 저자는 `attribute`를 정의하며 논문을 시작한다.
  * 기존의 pbl에서 사용되던 `feature function` $\phi (\zi)$ 와 유사하게, `attribute`라는 용어를 정의하며, 이는 behavior에 대해 symbolic concept을 가진다. (`e.g., increasing the softness or speed of agents’ movement`)
...
* $\zi_{\sigma}(\tau ', \alpha_{i}) \simeq \boldsymbol{v}^{\alpha_{i}}_{t}$: target attribute score $\boldsymbol{v}$에 대한 mapping function $\zi$
<br />
