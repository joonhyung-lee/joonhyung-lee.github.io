...
* $X \sub \mathbb{R}^{d}$: state space, where $d\in \mathbb{N}$ is the dimensionality of the state space.
  * $d_{w}\in \mathbb{N}$: workspace dimensionality
* $X_{\text{obs}} \sub X$: obstacle state space
* $X_{\text{free}} \sub X\textbackslash X_{\text{obs}}$: obstacle state space
  * Initial state $x_{\text{init}} \in X_{\text{free}}$, goal region be $X_{\text{goal}} \sub X_{\text{free}}$
