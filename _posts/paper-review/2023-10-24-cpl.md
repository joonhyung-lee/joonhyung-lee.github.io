...
tags:
  ...
  - Arxiv
  - '2023'
excerpt: "paper review about CPL"
use_math: true
classes: wide
---
...
$$
\begin{equation}
    P_{A^*}\left[ \sigma^{+} \succ \sigma^{-} \right] = \frac{\exp \sum_{\sigma^{+}} \gamma^{t} A^{*}(s^{+}_{t}, a^{+}_{t})}{\exp \sum_{\sigma^{+}}\gamma^{t} A^{*}(s^{+}_{t}, a^{+}_{t}) + \exp \sum_{\sigma^{-}}\gamma^{t} A^{*}(s^{-}_{t}, a^{-}_{t})}
\end{equation}
$$
...
* $\mathcal{D}_{\text{pref}}$: Informations about the optimal advantage function $A^{*}(s,a)$

$$ \argmax_{a \in \mathcal{A}} A^{*}(s,a)=a^{*} $$
그러므로 정의에 따라 $A^{*}(\cdot)$를 최대화하는 action은 optimal action $a^{*}$가 되며 이 $A^{*}(\cdot)$ 함수를 학습한다는 것은 결국 optimal policy $\pi^{*}$를 반드시 따른다는 것을 알 수 있으며 이는 아래의 수식으로 표현 가능하다.
...
$A^{*}$를 $\pi^{*}$에 대한 관계는 아래와 같다.[[Ziebart (2010)](https://www.cs.cmu.edu/~bziebart/publications/thesis-bziebart.pdf)]  $\pi^{*}$는 확률 분포의 정의에 따라 $A^{*}$도 몇 가지 제약 사항을 충족해야만 한다. ([probability axioms](https://www.google.com/search?q=axiom+probabilty&oq=axiom+probabilty&aqs=chrome..69i57.2897j0j7&sourceid=chrome&ie=UTF-8))
$$
\begin{equation*}
    \pi^{*}(a|s) = \exp^{\frac{A^{*}_{r}(s,a)}{\alpha}}
\end{equation*}
$$

이는 학습된 $A^{*}$가 optimal 하려면, 확률 분포 정의에 의해 normalized 되어야 한다는 것이다. $ \int_{\mathcal{A}} \exp^{\mathcal{A}^{*}(s,a)/\alpha}~da = 1$. 이러한 제약 조건은 깊은 신경망과 continuous space 일 경우에 intractable 하게 되므로 $A_{\theta}$를 통해 학습하는 접근(`=MLE`)은 어려워진다.

그렇다면 $A_{\theta}$를 직접 학습하는 것은 지양해야 한다는 것이며, 다른 방법을 찾아야 한다.  저자는 $A^{*}$와 $\pi^{*}$의 bijection 성질을 이용해서 수식을 전개해나간다. 이를 통해 $A^{*}$이 $\pi^{*}$에 **log-scale**로 비례한다는 의미로 수식을 다시 작성할 수 있다.

$$
\begin{equation}
    A^{*}_{r}(s,a) = \alpha \log \pi^{*}(a|s)
\end{equation}
$$
...
$$
\begin{equation}
    P_{A^*}[\sigma^{+} \succ \sigma^{-}] = \frac{\exp \sum_{\sigma^{+}} \gamma^{t} \alpha \log \pi^{*} (a^{+}_{t} | s^{+}_{t})}{\exp \sum_{\sigma^{+}}\gamma^{t} \alpha \log \pi^{*} (a^{+}_{t} | s^{+}_{t}) + \exp \sum_{\sigma^{-}}\gamma^{t} \alpha \log \pi^{*} (a^{-}_{t} | s^{-}_{t})}
\end{equation}
$$

이렇게 MaxEnt RL에서 $\pi^{*}$ 만으로 해석할 수 있는 preference model을 유도하게 된다. 위 수식을 토대로 `Maximum-Likelihood Estimation(MLE)` Loss도 손쉽게 유도할 수 있다.

$$
\begin{equation}
    \mathcal{L}_{\text{CPL}}(\pi_{\theta}, \mathcal{D}_{\text{pref}}) = \mathbb{E}_{(\sigma^{+},\sigma^{-})\sim \mathcal{D}_{\text{pref}}}\left[ -\log \frac{\exp \sum_{\sigma^{+}} \gamma^{t} \alpha \log \pi^{*} (a^{+}_{t} | s^{+}_{t})}{\exp \sum_{\sigma^{+}} \gamma^{t} \alpha \log \pi^{*} (a^{+}_{t} | s^{+}_{t})+\exp \sum_{\sigma^{-}} \gamma^{t} \alpha \log \pi^{*} (a^{-}_{t} | s^{-}_{t})}  \right]
\end{equation}
$$
...
> Assuming sufficient representation power, at convergence $\pi_{\theta}$ will perfectly model the users preferences, and thus exactly recover $\pi^{*}$ under the advantage-based preference model given an unbounded amount of preference data.
...
3. $\int_{\mathcal{A}} \exp^{\mathcal{A}^{*}(s,a)/\alpha}~da = 1$ 성질에 의해 어떠한 reward function에 상응하는 optimal advantage function $A^{*}$이 존재한다.
...
$$
\begin{equation}
    \mathcal{L}_{\text{CPL}\color{red}({\lambda})}(\pi_{\theta}, \mathcal{D}_{\text{pref}}) = \mathbb{E}_{\mathcal{D}_{\text{pref}}}\left[ -\log \frac{\exp \sum_{\sigma^{+}} \gamma^{t} \alpha \log \pi^{*} (a^{+}_{t} | s^{+}_{t})}{\exp \sum_{\sigma^{+}} \gamma^{t} \alpha \log \pi^{*} (a^{+}_{t} | s^{+}_{t})+\exp {\color{red}({\lambda})}\sum_{\sigma^{-}} \gamma^{t} \alpha \log \pi^{*} (a^{-}_{t} | s^{-}_{t})}  \right]
\end{equation}
$$
* $\lambda \in (0,1)$
