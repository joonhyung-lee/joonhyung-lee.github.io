...
A classic way for this task is to choose **template vectors** $\bold{c_{k}}$, which represents a typical sound in each environment $\bold{k}$. To categorize the sounds, you then find that **template vector** which is closest to your recording $\bold{x}$. In mathematical notation, you search for a $\mathbf{ k^{^*} }$ by

$$ \mathbf{k^*} = \arg\min_{\bold{k}} \|x-c_{\bold{k} }\|^2. $$

The above expression thus calculates the squared error between $x $ and each of the vectors $c_{\bold{k}}$ and chooses the index $\bold{k}$ of the vector with the smallest error. The vectors $c_{\bold{k}}$ then represent a codebook and the vector $x$ is quantized to $c_{\bold{k}^*}$. This is the basic idea behind *vector quantization,* which is also known as $\bold{k}$**-means**.
...
$$ E_h\left[ \min_\bold{k} \|x_h-c_\bold{k}\|^2 \right] \approx \frac 1N \sum_{h=1}^N \min_\bold{k} \|x_h-c_\bold{k}\|^2, $$
...
$$ \{c_\bold{k}^*\} := \arg\min_{\{c_\bold{k}\}}\, E_h\left[ \min_\bold{k} \|x_h-c_\bold{k}\|^2 \right]  $$

or more specifically, for a dataset as

$$ \{c_\bold{k}^*\} := \arg\min_{\{c_\bold{k}\}} \sum_{h=1}^N \min_\bold{k} \|x_h-c_\bold{k}\|^2. $$
...
1.  For every vector $x_{h}$ in a large database, find the best codebook vector $c_{\bold{k}}$.
2.  For every codebook vector $c_{\bold{k}}$; 
...
To improve performance, we can apply several heuristic approaches. For example, we can start with a small codebook $ \{ c_\bold{k} \}_{\bold{k}=1}^\bold{K} $ of $\bold{K}$ elements and optimize it with the EM algorithm. We then split the codebook into two, offset by a small delta $d$, such that $ \|d\|<\epsilon $ and make the new codebook $ \{ \hat c_\bold{k} \}_{\bold{k}=1}^{\bold{2K}} := \{ c_\bold{k},\, c_\bold{k}+d \}_{\bold{k}=1}^\bold{K} $ of $\bold{2K}$ elements.
...
The advantage of this approach is that it focuses attention to the big bulk of datapoints $x_{\bold{k}}$, and ignores outliers. The outcome is then expected to be **more stable and the likelihood of converging to a local minimum is smaller.** The downside is that with this approach it is then more **difficult to find small separated islands(=subsets).** That is, because the initial codebook is near the center of the whole mass of datapoints, adding a small delta to the codebook vectors keeps the new codevectors near the center-of-mass. 

Conversely, we can start with a large codebook, say treat the whole input database $x_{\bold{k}}$ as a codebook. We can then iteratively merge pairs of points which are close to each other, until the codebook is reduced to the desired size. Needless to say, this will be a slow process if the database is large, but will be very efficient in finding separated islands of points. 
